 ...
 * 
 */
public class PermitJoinResource extends CommonResource {
	...
	public void processPost(String body) {
		...
		Address address = getAddressAttribute(Resources.PARAMETER_AOI);
		...
		try {
			...
			if (uriListener == null) {
				...
				Status status = gatewayInterface.permitJoinSync(timeout, address, joiningInfo.getPermitDuration());
				sendStatus(status);
			} else {
				...
				gatewayInterface.permitJoin(timeout, address, joiningInfo.getPermitDuration());
				sendSuccess();
			}
		} catch (Exception e) {
