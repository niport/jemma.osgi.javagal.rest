 ...
 * 
 */
public class GetServiceDescriptorResource extends CommonResource {
	...
	public void processGet(String body) {
		...
		short endpointId = this.getShortAttribute(Resources.URI_ENDPOINT);

		try {
			if (uriListener == null) {

				gatewayInterface = getGatewayInterface();
				ServiceDescriptor sd = gatewayInterface.getServiceDescriptorSync(timeout, address, endpointId);

				Detail details = new Detail();
				details.setServiceDescriptor(sd);
				...
				client.getClientEventListener().setServiceDescriptorDestination(uriListener);
				gatewayInterface.getServiceDescriptor(timeout, address, endpointId);
